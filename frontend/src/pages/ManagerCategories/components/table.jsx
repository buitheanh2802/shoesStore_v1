        ...
        <table className="w-full text-md bg-white shadow-md rounded mb-4">
            <tbody>
                <tr className="border-b">
                    ...
                    <th className="text-left p-3 px-5">Tên danh mục</th>
                ...
                {categories.map((item, key) => {
                    return <tr key={key} className="border-b hover:bg-orange-100 bg-gray-100">
                        ...
                        <td className="p-3 px-5 font-medium">{item.categoryName}</td>
                        <td className="p-3 px-5"><img className='w-16' src={item.categoryImage.avatar} alt="hình ảnh sản phẩm" /></td>
                        <td className="p-3 px-5 flex justify-start my-auto">
                            <div>
                                <Link to = {`/admin/categories/edit/${item._id}`} type="button" className="mb-1 justify-center flex bg-yellow-500 focus:outline-none h-8 hover:bg-yellow-600 mr-3 px-2 py-1 rounded text-sm text-white w-20">
